...
import { listFollowedDistilleryIds } from '$lib/server/distillery-followers';
...
export async function load({ cookies }): Promise<{
	user: UserData | null;
	favorites: string[];
	distilleryFollowers: string[];
	view: ProductView;
}> {
	const user = await getSessionUser(cookies);
	const favorites = user ? await listFavoriteIds(user.id) : [];
	const distilleryFollowers = user ? await listFollowedDistilleryIds(user.id) : [];
	const v = cookies.get('rareold.view');
	const view: ProductView = v === 'list' || v === 'compact' ? v : 'grid';
	return { user, favorites, distilleryFollowers, view };
}
