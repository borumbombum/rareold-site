...
<header class="sticky top-0 z-40 border-b border-zinc-200/70 bg-white/80 backdrop-blur-md dark:border-zinc-800/70 dark:bg-zinc-950/80">
	<div class="mx-auto flex max-w-7xl flex-wrap items-center gap-x-3 gap-y-2 px-4 py-2.5 sm:px-6">
		...
		>
			<Menu size={18} />
			{#if filterActive}
				<span class="absolute right-1 top-1 h-2 w-2 rounded-full bg-accent ring-2 ring-white dark:ring-zinc-950"></span>
			{/if}
		</button>
		...
		<div class="order-2 w-full sm:order-none sm:w-auto sm:flex-1 sm:max-w-xl">
			<SearchBar />
		</div>
