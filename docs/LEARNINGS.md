...
- **Agitator premise damage:** the "double double maturation" idea and the distillery-location/flagship guesses were wrong; verified facts: **Agitator Destilleri AB** (Arboga, founded 2015 under Nordic Whisky Capital, Sweden's best-selling whisky brand), vacuum distillation (~0.3 bar) + chestnut casks (banned in Scotland), flagship now "The Swedish Malt" bottled at **500 ml@43%** (Systembolaget nr 5408902, EAN 7350107460409) — the 70cl editions are pre-2022. Only a Spanish exact review exists in supported locales (La Whiskería–Costa Rica); zero en/pt/fr/ja → ship es 1.
- **New precedent — exact-expression-empty product with distillery video:** Fary Lochan SÆSON 2 has **zero genuine review videos in any language** (only SÆSON 1, the Distillery Edition, single casks, a DR documentary and promos exist). User opted to ship it anyway with the verified Whisky Shared "Distillery Edition #08" video filling the slot (rule exception, user-approved).
- **SÆSON 2 spec corrections against the official page:** 2 × 1st-fill bourbon barrels (200 l) + 1 × 2nd-fill oloroso hogshead (250 l), first batch 1014 bottles (11.04.2025), **48%** (not 47%), and it is **nettle-smoked** (ortiga) — not peated. The Fifty Best 2025 PNG was the usable official image.
- **The Lakes WR No.7:** series concluded with No.7 (Sept 2023) at **52%** — the whole Reserve is batch-strength, not the 46–47.8% guess. New `lakes` distillery record (2011, Bassenthwaite/Cockermouth, Dhavall Gandhi's sherry-led style, first spirit Dec 2014, first release 2019).
- **Alberta Premium CS is batch-variable:** 2019 65.1% (the WOTY, Whisky Bible 2021 = 97.5), 2020 66.0%, 2021 63.7%, 2022 63.5%, 2023 64.2% (current). Seeded the iconic **65.1% / 750 ml** with the WOTY framing since cask-strength ABV is batch-specific.
- **Bastille 1789 is NOT Maison Ferrand** — it's **Maison Daucourt** (Angoulême, Charente; 4th-generation family; brand launched 2010). The blend is ~50% malted barley + 50% raw wheat, double-distilled in Charente gooseneck pot stills with a triple cask finish (ex-bourbon + ex-Sauternes + ex-coñac) at **40%**; the single malt sits at 43%. Live site is bastillewhisky.com (bastille1789.com/.fr are dead).
- **Non-slot language yields:** every corpus produces extras that can't be seeded (sv, ko, de, da) — Swedish and Korean Solera/No.7/Agitator/Lakes reviews were verified but are not supported locale slots; only en/es/pt/ja/fr map onto the model.
- **Video totals this batch:** Agitator 1 es; SÆSON 2 1 en (distillery video); Solera 6 en (Whiskey Novice, Nitin Sood, TopWhiskies, Ash Parmar, "Was It Worth It", therenegade79); Lakes 2 en; Alberta 4 en + 1 es + 1 pt; Bastille 2 en + 2 pt + 2 ja. All 22 IDs double-verified via yt-verify.mjs.
- **Queue-edit slips:** my Canada section edit trashed the adjacent Canadian Club line and duplicated one — caught by a plain dup-scan before commit. Lesson reinforced: `grep ✓ lines | sort | uniq -d` after every queue-file edit.
...
- **Comparison videos are the slip-through failure mode even after a full agent pass.** Three non-exact videos were already in the seed: Michter's es "Small Batch & Rye" (two expressions tasted), Michter's ja "バーボン＆ライ", Peerless es "+Single Barrel". Removing them is cheap once you batch-audit every video title against "exact expression + in-language" in one oEmbed pass (39 IDs) instead of trusting per-product agent output.
- **When a violating video is removed, verify a replacement exists before substituting.** Michter's es got `EnumlIWBD5E` (Hablando de Whisky, "Cata Mitcher's Small Batch Bourbon", Spanish) — oEmbed-confirmed. Peerless es has NO exact small-batch video in Spanish after 4 search sources (native YT + 2 Invidious queries); left empty so English tops up at runtime. Same rule as always: never pad.
- **Official pages beat aggregators for proper specs.** Dareringer's own product page lists "PROOF 93 (46.5% ABV)" while the seed had 45%. Peerless's own tasting page + Bourbon Review (barrel proof ~109) confirmed 54.55%. Also confirmed Willett Pot Still "wheated" claim via official 2023 sell sheet (65% corn / 20% wheat / 15% barley) — the earlier "65/20/15 corn/rye/barley" from Secret Whiskey Society was wrong on the grain.
- **After any manual DB correction the full chain must be re-run**: data:export (397 whiskies / 172 distilleries / 4042 videos) → `npm run check` (0 errors / 29 baseline warnings) → re-verify ABV + video counts in the exported JSON with the array unwrapped (`w.find` fails on the nested `distilleries`/`whiskies` key).
...
## 2026-09-05 — Batch: 5 more blends (J&B Rare, Whyte & Mackay Triple Matured, Bell's Original, William Lawson's, Big Peat)

- The tiering rule holds: nothing in this batch filled fr (all 5 honest-dry for French — French reviewers only cover the top-tier blends). es filled for J&B (4), W&M (2), Bell's (2), Lawson's (4) but NOT Big Peat: the only Spanish hits were Big Peat Christmas Editions / 12yo / Douglas Laing roundups — no exact core-release es review exists, so es=0 shipped.
- Comparison "fight" videos (Tierri's Teacher's vs W&M) taste the exact whisky but often poison the language/expression gate: dropped ItammMuYcCk (already live on teacher's product, title never names "Triple Matured"). One URL per product is the rule; reusing across products for a vs-video is defensible but weak — dropped it anyway. Whyte & Mackay pt shipped at 1 (precedent: vat-69 pt:1 in task 054), floor 2 is a guideline under honest-dry coverage.
...
## 2026-09-06 — Batch: Hedonism, Haig Club Clubman, Girvan No. 4, Jameson Caskmates Stout, Jameson Black Barrel

- Tiering held again: **Jamesons are mass-market → the richest coverage (en4/es4/pt2/ja4 each, es fills first time in a while at 4/4)**, Haig Clubman mid (en4/es3/ja2/fr1, pt dry), Hedonism thin (en4/pt1/ja1, es+fr dry), Girvan No. 4 thinnest of all (en1 only — it's a niche bottle of a grain distillery with a near-zero review footprint). es/pt Coverage correlates with brand fame, exactly as in blends batches.
- **Invidious's ID↔title mapping is scrambled (search-only endpoint)**: never trust an invidious line for *which* video an ID is; only oEmbed (`yt-verify.mjs`) is authoritative for pairing an ID with its channel+title. Invidious is a discovery net, oEmbed is the audit.
- **English auto-translation of titles in native search results hides the spoken language** — a Japanese channel with an English title ("The COMPASS BOX Collection Reviewed") is not in-language; verify via channel + oEmbed original title.
- **A French Whiskey Tribe "8 Jameson" and Greg's Whisky Guide "Caskmates Stout" both turned out English** once verified — the fr slots for stout/bb/hessay stayed honest-dry rather than accepting auto-dubbed-looking candidates whose real channel said otherwise.
- Distillery facts quickly verifiable from distillery.com/official sources (Cameronbridge 1824 John Haig Fife; Girvan 1963 William Grant Ayrshire, vacuum "Apps" still No. 4 from 1992 — the product's name comes from the still).

## 2026-09-06 — Batch: 5 Midleton Irish whiskies

- **Midleton = Ireland's "famous-first" tier again:** Redbreast 12, the flagship pot still, filled all five languages (en4/es4/pt4/ja4/fr1) — rivaling the Jamesons for es+pt richness (pt 4/4). Irish-focused channels (Whiskey Vault, Whiskey is a Journey, Los Whiskochos, Márcio Becker, ドラムズ・ジャパン, Spirituosités) cover the flagships across languages, and that depth evaporates for the variants.
- **Variant coverage falls off the flagship:** Lustau (46%, sherry finish) and Cask Strength showed the same honest-dry profile as niche blends — en + strong es (3), with fr/pt/ja showing up only where a single channel happened to review the exact variant (Spirituosités RB12, lachaineduwhisky RB12 CS, El Whisky Bar Jameson 18, ひとくちウイスキー Powers). Rule of thumb: for a mass-market brand's *flagship*, chase 4/4/4/4/4; for its *variants*, expect es+en then thin.
- **The exact-expression gate gets stricter the more variants a brand has:** Powers Gold Label is repeatedly conflated with Powers John's Lane; Jameson 18 with Jameson 18 Bow Street; Redbreast 12 with the Lustau/Cask Strength variants. Searching the full official name ("Jameson 18 Year Old", "Redbreast 12 Cask Strength") plus the variant keyword ("Lustau") is what surfaces the right videos and buries the sibling expressions.
- **A leading-dash video ID (`-SzAnRWpaB0`) breaks argv parsing** if you pass IDs as CLI args to `yt-verify.mjs`; piping via stdin (`cat ids.txt | node scripts/yt-verify.mjs`) is parse-safe and also one command for all 43.
- Comparison-battle acceptance rule used this batch (see lessons-learned): lead-subject comparison = keep; balanced multi-whisky battle elsewhere than the product = drop.
## 2026-09-07 — Batch: 5 Irish whiskies (Method & Madness SPS, Teeling x3, Roe & Co)

- **Label generation is part of data integrity, not prose.** When I drafted seed labels for the Roe & Co videos from subagent summaries (which had truncated the en/es/ja channel names), three ja channel names were wrong. The lesson: make the FINAL label writing a mechanical step that consumes `yt-verify.mjs` output (oEmbed channel + title) — never fill labels from memory or partial output. Re-verification must happen with the seed file already drafted, so you correct what you actually shipped.
- **The exact-expression gate has a third failure mode — brand-launch-era reviews.** The first two are variant-confusion and wrong-language. The third: a distillery's launch reviews review its *poitín / new-make / Spirit of Dublin* bottling, which looks topically on-target ("Teeling") but is not the matured product. Keyword combo that saves the search: full product name + "Single Pot Still" (the style name), not just the brand.
- **TWE-CDN reliability rule from past batches held again, but this batch proved a non-Shopify alternative:** official brand CDNs (Teeling cloudfront, Roe & Co Contentful, whiskyrant) are also 403-proof sources that aren't Whiskybase. Lesson crystallized: Whiskybase static = always blocked; the fallback ladder is brand-CDN → TWE → Shopify → any 200 image that cleans to 500×500 webp.

## 2026-09-09 — Batch: 5 Kentucky bourbons (Basil Hayden's, Booker's, Baker's 7, Maker's Mark, Maker's 46)
...
## 2026-09-17 — Israeli batch: seed-builder bug + libsql undefined args + auto-regions

- **JSON.dumps/stringify silently drops `undefined`** — a builder that assembles a product object without `volume`/`abv`/`region` writes the rows KEYLESS, and the insert's bound args then contain `undefined`, which @libsql/hrana rejects at client-side binding (`TypeError: Unsupported type of value` in `value.js:38`), NOT at the DB. The TypeError masks the cause — dig for the `undefined` arg. Reproduction trick: build the exact bound array and `await c.execute()` per row; because the toProto conversion is asynchronous in the http transport, a sync try/catch prints "OK" for every row while the promise rejects silently. Await first.
- **db-sync region auto-derivation:** `regionNamesByOrigin` + `slugify` builds regions purely from seed whisky `region` values — adding a whisky with a new region string J-cases region rows into existence (`${origin}-${slug}`). No seed file, no migration needed.
- **Reviewer-coverage floor for niche distilleries:** Golani (10yo, 192-bottle microrelease) and Ruach (Pardes Hanna) have essentially zero exact-expression reviews on YouTube — only the distillery's own channel, WhiskyLive interviews, and new-make reviews. Strict rule leaves them videoless; product still ships.

## 2026-09-18 — Taiwan batch: invidious title instability at scale + zero-yield languages are fine

- **Invidious search results are unstable, and the dedup in `yt-invidious.mjs` can pair a video ID with the wrong result** — several consecutive calls returned the same ID with different titles (NycqtzlveTk appeared as "ウイスキーレビュー97：カバラン ソリスト フィノシェリーカスク" in one run but its oEmbed title is a Kavalan-brand-ambassador interview; mhy_o5_0zaI flip-flopped between Oloroso and Fino). The same ID string repeated across lines in one output can actually be several different videos. Rule: invidious output is only a discovery hint — every single pick must be confirmed via oEmbed (actual `yt-verify` step).
- **Naming collision trap:** Kavalan "Sherry Oak" (older core SKU, all over PT/ES reviews e.g. Bebendo Whisky 129, Primewhisky) is a DIFFERENT SKU from "Oloroso Sherry Oak" — keep them separate under the exact gate; don't let a "Sherry" review land on an "Oloroso" product.
- **Japanese channels cover the Kavalan core range almost 1:1** (はっちばっちチャンネル: Oloroso Sherry Oak, Bourbon Oak, Concertmaster, Classic = the whole core lineup). Oracle: when you find one channel's "series", query the other core expressions through them.
- **Product with zero videos still ships** — OMar Cask Strength (Bourbon, 55.7%) has no dedicated exact review on YouTube (only Whiskey Vault's Black Queen Wine Finish, which is a different finish, + Gwhisky's multi-bottle roundup = both rejected). This is the third consecutive batch with a videoless SKU (Golani, Ruach, now OMar CS).

- **Agitator "The Swedish Malt" = the core/launch single malt** (43%, bottle code ASM-U303, "Agitator Single Malt Whisky" on Whiskybase). Videos titled "Agitator Single Malt" (BRsweden per-flask tasting, La Whiskería's dedicated brand-ambassador video) DO hit this SKU; but the same label on Invidious results mispaired IDs — always re-verify via oEmbed. Only sv (Whisky Weekly "Äntligen premiär", Whisky Kalendern "premiär buteljering") + es (La Whiskería "Conociendo Agitator Single Malt.") confirmed. **No dedicated English review exists** — many generic "Agitator whiskey review" titles (Brian's Bourbon, Moa Nilsson sample roundup) don't state the exact expression, so per the hard rule they're rejected; ship the slot empty (sv/es aren't in the allowed {es,en,pt,ja,fr} set, so this SKU effectively ships videoless too).

## 2026-09-19 — Fary Lochan SÆSON 2: zero-yield exhaustive search

- **SÆSON 2 (first batch of 1014 bottles, Apr 2025) has ZERO exact-expression reviews on YouTube.** Near-misses that must NOT ship under the hard rule: Whisky Shared's "Fary Lochan Season 1 Batch #01" (WBcLkulOANk, Oct 2024 = SÆSON 1, explicitly excluded), Whisky Whistle 765 Sherry Cask single (NhX-IgbnED4), Whiskyadventurer "Tasting a new release from Fary Lochan" (yFZvTWCpceE = Oct 2018, pre-SÆSON era), Pour & Sip Sept 2026 live (T4lHBuwiyh0 = box dram was a Fary Lochan 2016 MoM single cask 51.9%, not SÆSON 2), The Whisky Waiter's German Liquid Madness hogshead cask review (KJF_e083K6M), Distillery Edition #08 (N_mA_tRWSoc), DR documentary, distillery promos.
