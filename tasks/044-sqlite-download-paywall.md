...
- 2026-09-01: Public page superseded — the SQLite purchase UI moves to `/database` (task 066). The `/api/data/download`, `/api/download/request`, `downloads.ts`, `dbfile.ts` machinery described here stays unchanged and is reused by 066. `/download` itself is left untouched (no redirect).
